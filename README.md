...
**v1.26.0** — 설정에 테마 선택(라이트 / 다크 / 시스템, 기본 시스템)과 완료 소리(기본 끔) 추가 — 밀도·너비·알림 설정은 그대로
...
| **시리즈 완주** | YouTube 재생목록 / 품번(예: SSIS-001) 다음 편 제안 / 목록 페이지 링크 일괄 수집(받은 편 자동 표시·페이지네이션) |
| **품번 존재 검증** | 추정만 하지 않고 페이지를 확인해 **있는 편만** 목록에 표시 |
| **나중 받기** | 워치리스트, 시리즈 묶음 받기/삭제, 예약 |
| **로컬 서재** | 완료·실패 기록, 시리즈 이어받기(서재 최대 번호 이후) |
| **저장 공간 관리** | 서재 탭에서 실제 파일 용량·날짜 조회, 선택 파일 휴지통 이동 (도우미 연동) |
| **백업** | 설정·나중 받기·서재를 JSON으로 내보내고 가져오기 (합치기) |
...
- 링크 받기 시 도우미(yt-dlp)가 그 주소를 지원하는지 먼저 확인합니다 — 지원이면 페이지를 열지 않고 바로 받고, 미지원이면 수 초 내 재생 안내로 끝냅니다.
- HLS 용량 표시는 영상을 재생하는 동안 실제 세그먼트 크기를 관측해 점점 정확해집니다(세그먼트 실측 평균 기반).
- 도우미는 받기가 끝난 파일의 **손상 검사**(ffprobe, 설치 시 자동)를 거친 뒤 저장합니다 — 잘린 파일은 "손상 의심"으로 알려주고 다시 받도록 안내합니다.
- 팝업 상단의 **yt-dlp 업데이트** 버튼은 헬퍼에 `POST /update`(yt-dlp -U)를 요청합니다. Homebrew·pip 설치본은 자체 업데이트가 불가하므로 설치 명령을 안내해 줍니다.

### 보안

- 헬퍼는 `Origin` 헤더가 붙은 요청 중 **확장 프로그램(`chrome-extension://`) 출처만** 허용합니다. 일반 웹페이지가 헬퍼에 명령을 내리는 것을 차단합니다. `/health` 는 공개이고, 받기·삭제 등 변경 요청은 페어링(또는 `UVD_TOKEN`) 토큰이 필요합니다. `curl` 등 Origin 없는 로컬 도구도 같은 토큰을 보내야 합니다.
...
- 토큰 인증(선택): 헬퍼를 `UVD_TOKEN=<비밀값>`으로 실행하고, 확장에 같은 값을 넣으면 모든 요청에 토큰이 요구됩니다. 자동 페어링 토큰은 확장 페이지에서만 읽을 수 있는 `chrome.storage.session`에 저장됩니다.
